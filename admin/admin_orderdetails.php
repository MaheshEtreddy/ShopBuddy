...
if (isset ( $_POST ['submit'] )) {
	...
	echo "<script>window.location = 'admin/admin_orderdetails.php?cancelled'</script>";
}
...
<form action="" method="post">
    ...
    <?php
			...
			<option value=''>...Select...</option>";
						$OrderStatusarray = array (	"Order Confirmed", "Packed and Ready for delivery", "In Transit",
													"Cancelled by provider","Cancelled by customer","Returned","Incomplete");
						foreach ( $OrderStatusarray as $value ) {
							if ($value == $odrDateStatRes ['Ordercomments']) {
								echo '<option value='.$value.'  selected="selected">'.$value.'</option></select></td></td></tr>';
							} else {
